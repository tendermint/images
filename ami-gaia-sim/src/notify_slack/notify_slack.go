	...
	if *seed_failed {
		message = "Simulation with *seed " + *seed_num + "* failed! To replicate, run the command:  ```" +
			`go test -mod=readonly ./cmd/gaia/app -run TestFullGaiaSimulation \
								...
								-SimulationNumBlocks=` + *num_blocks + ` \
...
func send_message (slack_api *slack.Client, channel_id string, message string) string {

	_, ts, err := slack_api.PostMessage(channel_id, slack.MsgOptionText(message, false))
	if err != nil {
		fmt.Printf("%s\n", err)
	}
	return ts
